...
def get_sum_display_info(self, db:Session, sum_id: int)-> sum:
        if student_id:
            get_sum_display_info = db.execute(text(f'SELECT * FROM [dbo].[get_student_display_info] ({student_id})')).first()
            db.commit()
            if _sum and _sum[0]:
                return Student(**json.loads(_student[0])[0])
        return None
